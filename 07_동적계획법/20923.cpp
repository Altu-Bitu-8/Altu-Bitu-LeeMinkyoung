...
string judge(cards& deck) {
	...
	if (do_deck > su_deck) { // 도도의 카드가 더 많은 경우
		return "do";
	}
	else if (do_deck < su_deck) { // 수연의 카드가 더 많은 경우
		return "su";
	}
	return "dosu"; // 둘 다 같을때
}
...
void groundToDeck(deque<int>& deck, deque<int>& ground) {
	while (!ground.empty()) {
		deck.push_back(ground.back()); //그라운드 맨 뒤 카드를 덱 뒤에 추가
...
string game(int m, cards& deck, cards& ground) {
	...
	while (m--) {
		...
		if (bell != -1) { // 종을 울린 경우
			ringTheBell(bell, deck, ground); // 카드 이동
		}
		turn = !turn; // 차례 바꾸기
	}
	return judge(deck); // 승자
}
